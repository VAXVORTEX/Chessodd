...
static func create_ui(main: Node):
	...
	# --- SETTINGS UI ---
	main.settings_panel = ColorRect.new()
	main.settings_panel.color = Color(0.05, 0.05, 0.08, 0.98)
	main.settings_panel.mouse_filter = Control.MOUSE_FILTER_STOP
	main.settings_panel.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	main.settings_panel.z_index = 2000
	main.settings_panel.hide()
	main.settings_panel.process_mode = Node.PROCESS_MODE_ALWAYS
	main.ui_layer.add_child(main.settings_panel)
	
	var sp_vbox = VBoxContainer.new()
	sp_vbox.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
	sp_vbox.grow_horizontal = Control.GROW_DIRECTION_BOTH
	sp_vbox.grow_vertical = Control.GROW_DIRECTION_BOTH
	sp_vbox.add_theme_constant_override("separation", 40)
	sp_vbox.alignment = BoxContainer.ALIGNMENT_CENTER
	main.settings_panel.add_child(sp_vbox)
	
	main.lbl_settings_title = Label.new()
	main.lbl_settings_title.text = "Settings"
	main.lbl_settings_title.set("theme_override_font_sizes/font_size", 80)
	main.lbl_settings_title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	sp_vbox.add_child(main.lbl_settings_title)
	
	var tabs = VBoxContainer.new()
	tabs.custom_minimum_size = Vector2(800, 400)
	tabs.add_theme_constant_override("separation", 20)
	sp_vbox.add_child(tabs)
	...
	tabs.add_child(tab_btns)
	
	var content_area = PanelContainer.new()
	content_area.custom_minimum_size = Vector2(800, 350)
	tabs.add_child(content_area)
	
	var show_tab = func(tab_node):
		for c in content_area.get_children():
			c.hide()
		tab_node.show()
	
	var create_tab_btn = func(text, tab_node):
		var btn = Button.new()
		btn.text = text
		btn.add_theme_font_size_override("font_size", 40)
		btn.custom_minimum_size = Vector2(200, 60)
		btn.pressed.connect(func(): show_tab.call(tab_node))
		tab_btns.add_child(btn)
		content_area.add_child(tab_node)
		tab_node.hide()
		
	# Audio Tab
	var audio_tab = VBoxContainer.new()
	audio_tab.name = "Audio"
	audio_tab.alignment = BoxContainer.ALIGNMENT_CENTER
	audio_tab.add_theme_constant_override("separation", 30)
	
	var lbl_master = Label.new()
	lbl_master.text = "Master Volume"
	lbl_master.set("theme_override_font_sizes/font_size", 40)
	var slider_master = HSlider.new()
	var initial_st = SaveManager.load_settings()
	slider_master.value = initial_st.get("volume", 50.0)
	...
	audio_tab.add_child(lbl_master)
	audio_tab.add_child(slider_master)
	create_tab_btn.call("Audio", audio_tab)
	
	# Graphics Tab
	var gfx_tab = VBoxContainer.new()
	gfx_tab.name = "Graphics"
	gfx_tab.alignment = BoxContainer.ALIGNMENT_CENTER
	gfx_tab.add_theme_constant_override("separation", 30)
	
	var slider_gamma = HSlider.new()
	var slider_contrast = HSlider.new()
	var slider_saturation = HSlider.new()
	var slider_time = HSlider.new()
	var cb_clouds = CheckBox.new()
	var cb_shadows = CheckBox.new()
	
	var cb_fullscreen = CheckBox.new()
	
	var st = SaveManager.load_settings()
	
	var apply_live_visuals = func():
	...
	var save_visuals_func = func():
		...
		SaveManager.save_settings(TranslationManager.lang, cb_fullscreen.button_pressed, SaveManager.load_settings().get("volume", 50.0), 0)

	cb_fullscreen.text = "Fullscreen"
	cb_fullscreen.add_theme_font_size_override("font_size", 40)
	cb_fullscreen.button_pressed = st.get("fullscreen", false)
	gfx_tab.add_child(cb_fullscreen)
	
	var gamma_hbox = HBoxContainer.new()
	gamma_hbox.alignment = BoxContainer.ALIGNMENT_CENTER
	gamma_hbox.add_theme_constant_override("separation", 20)
	var lbl_gamma = Label.new()
	lbl_gamma.text = "Gamma"
	lbl_gamma.set("theme_override_font_sizes/font_size", 30)
	slider_gamma.min_value = 0
	slider_gamma.max_value = 200
	slider_gamma.value = st.get("gamma", 100)
	slider_gamma.custom_minimum_size = Vector2(300, 40)
	gamma_hbox.add_child(lbl_gamma)
	gamma_hbox.add_child(slider_gamma)
	gfx_tab.add_child(gamma_hbox)

	var contrast_hbox = HBoxContainer.new()
	contrast_hbox.alignment = BoxContainer.ALIGNMENT_CENTER
	contrast_hbox.add_theme_constant_override("separation", 20)
	var lbl_contrast = Label.new()
	lbl_contrast.text = "Contrast"
	lbl_contrast.set("theme_override_font_sizes/font_size", 30)
	slider_contrast.min_value = 0
	slider_contrast.max_value = 200
	slider_contrast.value = st.get("contrast", 100)
	slider_contrast.custom_minimum_size = Vector2(300, 40)
	contrast_hbox.add_child(lbl_contrast)
	contrast_hbox.add_child(slider_contrast)
	gfx_tab.add_child(contrast_hbox)

	var saturation_hbox = HBoxContainer.new()
	saturation_hbox.alignment = BoxContainer.ALIGNMENT_CENTER
	saturation_hbox.add_theme_constant_override("separation", 20)
	var lbl_saturation = Label.new()
	lbl_saturation.text = "Saturation"
	lbl_saturation.set("theme_override_font_sizes/font_size", 30)
	slider_saturation.min_value = 0
	slider_saturation.max_value = 200
	slider_saturation.value = st.get("saturation", 100)
	slider_saturation.custom_minimum_size = Vector2(300, 40)
	saturation_hbox.add_child(lbl_saturation)
	saturation_hbox.add_child(slider_saturation)
	gfx_tab.add_child(saturation_hbox)

	cb_clouds.text = "Enable Clouds"
	cb_clouds.add_theme_font_size_override("font_size", 30)
	cb_clouds.button_pressed = st.get("clouds", true)
	gfx_tab.add_child(cb_clouds)

	cb_shadows.text = "Enable Shadows"
	cb_shadows.add_theme_font_size_override("font_size", 30)
	cb_shadows.button_pressed = st.get("shadows", true)
	gfx_tab.add_child(cb_shadows)
	
	var btn_reset = Button.new()
	btn_reset.text = "Reset Graphics"
	btn_reset.set("theme_override_font_sizes/font_size", 30)
	btn_reset.custom_minimum_size = Vector2(300, 50)
	btn_reset.pressed.connect(func():
		slider_gamma.set_value_no_signal(100)
		slider_contrast.set_value_no_signal(100)
		slider_saturation.set_value_no_signal(100)
		cb_clouds.set_pressed_no_signal(true)
		cb_shadows.set_pressed_no_signal(true)
		apply_live_visuals.call()
	)
	gfx_tab.add_child(btn_reset)
	
	create_tab_btn.call("Graphics", gfx_tab)
	
	# Language Tab
	var lang_tab = VBoxContainer.new()
	lang_tab.name = "Language"
	lang_tab.alignment = BoxContainer.ALIGNMENT_CENTER
	lang_tab.add_theme_constant_override("separation", 30)
	
	var btn_en = Button.new()
	btn_en.text = "English"
	btn_en.set("theme_override_font_sizes/font_size", 40)
	btn_en.pressed.connect(func(): TranslationManager.set_locale("en"); main.update_ui_translation(); var s = SaveManager.load_settings(); SaveManager.save_settings("en", s.get("fullscreen", false), s.get("volume", 50.0), s.get("resolution", 0)))
	lang_tab.add_child(btn_en)
	
	var btn_ru = Button.new()
	btn_ru.text = "Русский"
	btn_ru.set("theme_override_font_sizes/font_size", 40)
	btn_ru.pressed.connect(func(): TranslationManager.set_locale("ru"); main.update_ui_translation(); var s = SaveManager.load_settings(); SaveManager.save_settings("ru", s.get("fullscreen", false), s.get("volume", 50.0), s.get("resolution", 0)))
	lang_tab.add_child(btn_ru)
	
	var btn_uk = Button.new()
	btn_uk.text = "Українська"
	btn_uk.set("theme_override_font_sizes/font_size", 40)
	btn_uk.pressed.connect(func(): TranslationManager.set_locale("uk"); main.update_ui_translation(); var s = SaveManager.load_settings(); SaveManager.save_settings("uk", s.get("fullscreen", false), s.get("volume", 50.0), s.get("resolution", 0)))
	lang_tab.add_child(btn_uk)
	create_tab_btn.call("Language", lang_tab)
	
	show_tab.call(audio_tab) # Default tab
	
	var btn_close_settings = Button.new()
	btn_close_settings.text = "Save & Close"
	btn_close_settings.set("theme_override_font_sizes/font_size", 50)
	btn_close_settings.custom_minimum_size = Vector2(400, 80)
	btn_close_settings.pressed.connect(func(): 
		apply_live_visuals.call()
		save_visuals_func.call()
		main.settings_panel.hide()
		if not main.pause_panel.visible:
			main.get_tree().paused = false
	)
	sp_vbox.add_child(btn_close_settings)
